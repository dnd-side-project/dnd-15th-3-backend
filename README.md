...
Mock 모드에서는 DB·OCI 스토리지를 초기화하지 않으며, Swagger(`http://localhost:3000/api/v1/docs`)와 고정 fixture를 제공합니다.
...
- `GET /api/v1/profile-avatars`: 프로필 캐릭터 목록
- `GET /api/v1/meeting-types`, `GET /api/v1/categories`, `GET /api/v1/places/search?keyword=성수`: 모임 생성 입력 화면
- `POST /api/v1/meetings/invitation/preview` (`{ "accessToken": "DNDFOR" }`): 초대 코드 검증 및 모임 미리보기
- `GET /api/v1/meeting/1?accessToken=host-session-token`: 방장용 모임 상세 화면
- `GET /api/v1/meeting/1?accessToken=member-session-token`: 참여원용 모임 상세 화면
