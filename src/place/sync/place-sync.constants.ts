...
export const PLACE_SYNC_RETRY_DELAYS_SECONDS = [60, 300, 1800] as const
...
export function buildCoverageTiles(
  ...
  const coverageDistance = radiusMeters + TILE_DIAGONAL_HALF_METERS
  const minLongitudeIndex = Math.floor(
    (centerX - coverageDistance) / PLACE_SYNC_TILE_SIZE_METERS,
  )
  const maxLongitudeIndex = Math.floor(
    (centerX + coverageDistance) / PLACE_SYNC_TILE_SIZE_METERS,
  )
  const minLatitudeIndex = Math.floor(
    (centerY - coverageDistance) / PLACE_SYNC_TILE_SIZE_METERS,
  )
  ...
  for (let yIndex = minLatitudeIndex; yIndex <= maxLatitudeIndex; yIndex += 1) {
    for (
      ...
      if (distance > coverageDistance) continue
